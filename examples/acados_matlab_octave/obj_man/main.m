...
model = object_manipulator_model(o.G, Fc_hat, dt, TAU_CONST);
% dims
...
lbu = -1000000*ones(nu, 1);
ubu = 1000000*ones(nu, 1);
if (TAU_CONST)
    lbh = -5000*ones(7,1);%zeros(7,1); % bounds on tau constraint
    ubh = 5000*ones(7,1);
end
%% acados ocp model
...
for ii=1:n_sim
    ...
    lambda_log(:,end+1) = x_sim(14:29,ii+1);
    Fc = o.Fc_hat*lambda_log(:,end);
    Fb = (o.G*Fc);
    %Fb = (M_o*(x0(7:12) - xp(7:12))/h + C_o*x0(7:12) + N_o);
    
    %(M_t*(x0(7:12) - xx(7:12,mpciter+1))/h + C_t*x0(7:12) + N_t);
    tau_log(:,end+1) = Jb'*(M_t*(x_sim(7:12,ii+1) - x_sim(7:12,ii))/dt + C_t + N_t);
