...
box on
set(gcf,'color','w');
exportgraphics(h,strcat(int2str(T*10),'error.pdf'));
...
exportgraphics(h,strcat(int2str(T*10),'lambda_min.pdf'));
...
exportgraphics(h,strcat(int2str(T*10),'lambda.pdf'));
...
exportgraphics(h,strcat(int2str(T*10),'tau_dot.pdf'));
...
set(l, 'Location', 'east');
exportgraphics(h,strcat(int2str(T*10),'q_dot_jl.pdf'))

%% cost value
h = figure('Renderer', 'painters', 'Position', [10 10 900 300])

plot(t(1:size(cost_val_ocp,2)), cost_val_ocp, 'k-', 'Linewidth', lw ,'Color', [0.0, 0.0, 0.0]);
hold on
...
box on
set(gcf,'color','w');
exportgraphics(h,strcat(int2str(T*10),'cost.pdf'));
